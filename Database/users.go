...
func GetAllUsers(currentUserID int64) ([]structs.User, error) {
	...
	rows, err := DB.Query(`SELECT u.id, u.username, u.email, u.created_at, u.status FROM users u LEFT JOIN messages m  ON ((m.from_user = u.id AND m.to_user = $1) OR (m.from_user = $1 AND m.to_user = u.id))  WHERE u.id != $1 GROUP BY u.id ORDER BY  COALESCE(MAX(m.created_at), '0000-00-00 00:00:00') DESC, LOWER(u.username) ASC`, currentUserID)
